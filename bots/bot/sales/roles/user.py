...
from bots.bot.sales.storage.keyboards.handlers import menu_choose
...
async def page_handler(message: types.Message, state: FSMContext):
    ...
    for order_id in orders:
        ...
        await message.answer_photo(image, order.text, "HTML", reply_markup=inline_keyboard)
    ...
    async with state.proxy() as data:
        ...
        for key, value in data["products"].items():
            if value != 0:
                ...
                id_page += 1
        await message.answer("press to submit order", reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("submit", callback_data="button_submit")))
...
async def buttons(callback_query: types.CallbackQuery, state: FSMContext):
   #todo 2
    ...
    s = session()
    if keyboard == "basket":
        await data.keyboards.handlers.basket.buttons(callback_query, state)
    # todo 1
    ...
    if name == 'plus' or name == 'minus':
        # menu_number = int(callback_query.storage.split("_")[2])
        # async with state.proxy() as storage:
...
def register_handlers(dp: Dispatcher):
    ...
    s = session()
    dp.message(welcome_handler, lambda id_user: check_user(s, id_user), commands="menu", state="*")
